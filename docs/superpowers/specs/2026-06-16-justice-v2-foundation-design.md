...
>
> **パスセグガシントの安全性（D69/D73・本レビュー Finding 4）**: 物理パス `events/<agentId>/<sessionId>/<writerId>.jsonl` の各セグァコードは、`agentId`（enum・D56）/`writerId`（`[A-Za-z0-9-]`・D55）/`sessionId`（後述）が安全である。具体的なアルゴリズム: (1) 許容文字は `[A-Za-z0-9_-]`、それ以外を `_` に置換、(2) 予約語 `.` / `..` / 空文字はそれぞれ `_dot_` / `_dotdot_` / `_empty_` に置換、(3) 単一セグァコードは **先に 64 文字へ truncation** し、(4) その後 **常に（衝突判定なしで）`sha256(sessionId)` の先頭 8 文字を hash suffix として付与**するため、最終 sessionId は「64 文字の本体 + 8 文字の hash suffix」で固定される（例: `ses_abc___a1b2c3d4`）。**D73 でこの hash suffix を既定で既に既後付与（无条件付与）に変更したことにより、異なる raw `sessionId` が同一物理セグァコードへ衝突する可能性は構造的に排除され、衝突検出用 alias map（`session-aliases.json` 等の永続サイドカー）は不要（実装上も存在しない）**。(5) 論理鍵 `shardId={agentId,sessionId,writerId}` と参照鍵には**生 `sessionId`** を保持し、エンコードは物理ファイル名生成にのみ適用。§9.4 NFR security と整合。

...
exact parent user-message chat entry. It decrements only the matching `(sessionId, command)` capture accounting. A
completion for A cannot mutate or delete B state.
...
The sum of pending capture credits for the four exact pinned commands MUST NOT exceed 8 for one session. Each of the
three identity maps (`chat.params`, finalized assistant, command completion) MUST NOT exceed 8 entries for one
session. The values are deliberately small because nominal operation requires one invocation and the supported
overlap proof requires two.

Overflow is fail-open for execution and fail-closed for audit attribution:
...
export type ReviewPostToolUsePendingRecord = {
  ...
  // These fields are omitted when the parent task PostToolUse wins the race
  // against durable child-session binding.
  readonly childSessionId?: string;
  readonly observedExecution?: ObservedReviewExecutionV1;
};
...
| artifact missing            | `artifact_missing`         | なし                         | 対応する `review_pending` / `final_review_pending` を blocked のまま保持する。claimed slot は terminalize する | 実行しない                         | 自動 review retry / rework / round変更なし。terminal後はdurable cleanup stateを評価し、recordなしまたはlatest `finished(cleanup_incomplete)` の場合だけprovider cleanupを開始/再評価する |
| artifact read failure       | `artifact_read_failed`     | なし                         | 対応する `review_pending` / `final_review_pending` を blocked のまま保持する。claimed slot は terminalize する | 実行しない                         | 自動 review retry / rework / round変更なし。terminal後はdurable cleanup stateを評価し、latest `finished(cleanup_incomplete)` 以外の既知outcomeはproviderを再実行しない                                                                           |
| artifact JSON invalid       | `artifact_json_invalid`    | なし                         | 対応する `review_pending` / `final_review_pending` を blocked のまま保持する。claimed slot は terminalize する | 実行しない                         | 自動 review retry / rework / round変更なし。terminal後はdurable cleanup stateを評価し、latest `finished(cleanup_incomplete)` 以外の既知outcomeはproviderを再実行しない                                                                           |
| artifact schema invalid     | `artifact_schema_invalid`  | なし                         | 対応する `review_pending` / `final_review_pending` を blocked のまま保持する。claimed slot は terminalize する | 実行しない                         | 自動 review retry / rework / round変更なし。terminal後はdurable cleanup stateを評価し、latest `finished(cleanup_incomplete)` 以外の既知outcomeはproviderを再実行しない                                                                           |
...
5. matching `PostToolUse` は、parent session と `callId` を current claimed slot に照合してから受理する。
   purpose と trusted correlation は event から読まず、slot と durable `TaskCallBinding` から解決する。
   parent session と `callId` が一致する parent `task` event は、artifact I/O の前に
   `ReviewPostToolUsePendingRecord` として既存 observation log に一度だけ記録する。child-session binding
   または `ObservedReviewExecutionV1` がまだ durable でない場合は artifact を読まずに
   `awaiting_child_binding` として返し、binding append 成功後、または restart recovery 時にこの pending
   record を再投影して同じ completion path を実行する。binding が durable になった後、Justice は
   child-session identity と observed execution を current slot / call identity に照合してから
   authoritative completion として扱う。slot / parent / call identity が一致しない event は stale advisory
   のみとし、pending marker を作らない。binding が durable になった後、Justice は terminalization 前に
...
| `claimed`（artifact read / validation failure staging） | `ReviewArtifactFailureStagingRecord` の trusted identity と failure reason を再検証し、対応する no-artifact failure terminal を一件だけ再試行する。terminal成功後は `ReviewArtifactCleanupRecord` をprojectし、recordなしまたはlatest `finished(cleanup_incomplete)` の場合だけcleanupを開始/再評価する。latest terminal statusまたはlatest `started`ではproviderを再実行しない。read-attempt marker がある限り artifact は再読しない。 | artifact の再読、ReviewArtifact / Gate / Acceptance、retry、round変更、同じ correlation の再dispatch |
...
- `callId`、parent session、purpose、trusted correlation、または durable child-session binding のいずれかが current claimed slot と一致しない `PostToolUse` は stale event として advisory のみを記録する。parent session / `callId` だけが一致し child-session binding がまだない event は、例外として `ReviewPostToolUsePendingRecord` に保持し、artifact を消費せず `awaiting_child_binding` のままにする。artifact を消費せず、`ReviewArtifactV1`、Gate、Acceptance、current review round に影響させない。
...
isolation properties. A reservation can only discover, inspect, restore, or retain its own quarantine entries;
recovery MUST NOT enumerate a global namespace and infer ownership from file names. Restore is
descriptor-relative and no-clobber, and recorded `st_dev` / `st_ino` identity is validated before
restore or retention classification. The supported v4.0.0 Linux provider does not physically unlink a
verified quarantine leaf. If the original path is occupied by a different inode, the quarantined object
is retained and the operation returns a non-positive cleanup outcome/advisory; the replacement is never
deleted or overwritten. Artifact and lease state may be re-evaluated together only when the latest
durable cleanup result is `cleanup_incomplete`, without crossing reservation boundaries.
...
// The descriptor is the only durable input accepted when a provider is
// reopened after process restart. It deliberately excludes status and
// review-correlation fields.
export type ReviewArtifactReservationDescriptor = {
  readonly artifactPath: string;
  readonly leasePath: string;
  readonly artifactIdentity: ReviewArtifactInodeIdentity;
};
...
- Justice は matching PostToolUse 到達後、`usable` な `TaskCallBinding.artifactReservation` に基づき、まず `ReviewArtifactReadAttemptRecord` を artifact I/O 前に一件だけ durable に記録し、その read lease に対して `artifactPath` を **一度だけ** 読み取る。既存の read-attempt record がある場合は path を再読しない。プロセス停止後に read-attempt はあるが completion / failure staging がない場合は、読み取り済みかどうかを推測せず `artifact_read_failed` として failure staging を作成し、artifact を再読しない。これにより crash 後の file mutation による別 digest の生成を許可しない。読み取り結果は strict schema validation を通し、reservation 単位の atomic consume operation として扱う。`unusable` の場合は artifact を読み取らず、mandatory review completion を成立させない。
  ...
  - 読み取り失敗は `ReviewArtifactFailureStagingRecord` を先に durable に記録してから、artifact payload を持たない `claimed → terminal` transition を一件だけ append する。failure staging または terminal append が失敗した場合は同じ durable marker を recovery で再試行し、read-attempt がある限り artifact を再読しない。terminal 成功後は matching durable terminal と reservation を cleanup identity に固定し、既存 Observation Log の `ReviewArtifactCleanupRecord` で physical cleanup lifecycle を管理する。record なしは cleanup 未実施として開始可能、latest `finished(cleanup_incomplete)` だけが restart 後の再評価対象、latest `finished(cleaned | quarantine_retained | replacement_retained)` は terminal、latest `started` は outcome uncertain として automatic provider retry 禁止とする。provider 呼出し前の `started` append が commit しなければ provider を呼ばず、provider 後の `finished(status)` append に失敗した場合も durable state は `started` のままなので再実行しない。cleanup outcome は terminal、Gate、Acceptance authority を rollback しない。
...
- `task_review` / `final_review` は matching parent、call、trusted correlation、category を
  durable projectionから検証し、child binding が存在する場合だけ Review Completion の artifact
  consume / terminalization path に進む。binding がまだない場合は pending marker を残して
  `awaiting_child_binding` で止める。
...
Review completion is triggered by the parent `task` tool's matching `PostToolUse` event
(`parentSessionId` plus the claimed parent `callId`). The child worker's artifact `write` is a
separate `PreToolUse` operation, and its `PostToolUse` event is never passed to the completion
consumer. If the parent `PostToolUse` arrives before `DelegatedExecutionBinding`, the pending
record omits child-specific fields and the consumer returns `awaiting_child_binding` without

...
- OpenCode Plugin 初期化コンテキスト:
  ...
  export const JusticePlugin: Plugin = async ({ project, client, $, directory, worktree }) => {
    return {
...
| 制約 | 実現手段 |
|---|---|
| Pure Core 保全 | `src/core/*.ts` は `@opencode-ai/plugin` を import しない。import してよいのは `src/opencode-plugin.ts` と `src/runtime/opencode-adapter.ts` のみ |
...
| `experimental.session.compacting` | `onSessionCompacting` | `EventEvent<CompactionPayload>` | `inject` → `output.context[]` にプランスナップショットを push |
| `session.error` | `onSessionError` | パターン一致時のみ `EventEvent<LoopDetectorPayload>` 相当 | 副作用のみ (plan.md 注記 + 次回 `tool.execute.before` で反映) |
...
- 返却ハンドラオブジェクトが 5 キーを持つ (`message.updated`, `tool.execute.before`, `tool.execute.after`, `experimental.session.compacting`, `session.error`)
...
```text
// OpenCode 側の設定
import { JusticePlugin } from "@yohi/justice/opencode";
...
| Adapter とレガシーシムが同時起動 | 二重初期化 / 二重 wisdom 書き込み | ドキュメントで相互排他を明記。相互排他チェックはコード実装せず運用ルールとする |

...
| `src/core/wisdom-store.ts` | `attachMetrics(metrics)` / `onEvict(listener)` / `updateMetrics(entryId, mutator)` を追加。`updateMetrics` は copy-on-write で当該 id のエントリを新インスタンスに差し替える唯一の経路。`getRelevant()` 時に `WisdomMetrics.recordHit` を呼ぶ。 |
...
class WisdomStore implements WisdomStoreInterface {
  ...
  getRelevant(options?): WisdomEntry[] {
    ...
    if (this.metrics) {
      const now = new Date();
      for (const entry of results) this.metrics.recordHit(this, entry.id, now);
    }
    return results;
  }

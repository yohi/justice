...
|------|--------|---------------|-------|
| `src/core/types.ts` | Modify | `FileWriter` インタフェースに `rename()` を追加 | Phase 1 Task 1 |
| `src/runtime/node-file-system.ts` | Modify | `rename()` 実装（path traversal 防御込み） | Phase 1 Task 1 |
| `tests/helpers/mock-file-system.ts` | Modify | `createMockFileWriter` に `rename` サポート追加 | Phase 1 Task 1 |
| `tests/runtime/node-file-system.test.ts` | Modify | `rename` の挙動テスト追加 | Phase 1 Task 1 |
...
| `src/core/wisdom-persistence.ts` | Modify | `saveAtomic()` + `mergeById()` 追加（既存 `save()` は保持） | Phase 1 Task 3 |
| `tests/core/wisdom-persistence-atomic.test.ts` | **Create** | atomic write / merge / LRU / rename 失敗テスト | Phase 1 Task 3 |
...
> **Milestone:** `FileWriter.rename` / `WisdomStore` の pure additions / `WisdomPersistence.saveAtomic` が完成し、既存機能を壊さずに atomic 書き込み基盤が利用可能。Phase 2 以降から再利用可能な独立した単位として `master` マージ可能。

### Task 1: `FileWriter.rename` 追加と `NodeFileSystem` 実装

**Files:**

- Modify: `src/core/types.ts`（`FileWriter` インタフェース）
- Modify: `src/runtime/node-file-system.ts`（`rename()` 実装）
- Modify: `tests/helpers/mock-file-system.ts`（mock writer に rename 追加）
- Modify: `tests/runtime/node-file-system.test.ts`（rename テスト追加）
...
- [ ] **Step 3: `NodeFileSystem.rename` の失敗テストを作成 (RED)**
...
- [ ] **Step 5: `FileWriter` インタフェースに `rename` を追加**
...
- [ ] **Step 6: `NodeFileSystem.rename` を実装**

`src/runtime/node-file-system.ts` を修正。import に `rename as fsRename` を追加し、クラスにメソッドを追加:

```typescript
import { mkdir, readFile, writeFile, stat, realpath, rename as fsRename } from "node:fs/promises";
```
...
> **Note:** `resolveSafelyForWrite` を使う理由は、`to` 側がまだ存在しない可能性があるため。既に path traversal 防御は `resolveSafelyForWrite` 内で実装済み。

- [ ] **Step 7: モックに `rename` を追加**
...
Run: `bun run test tests/runtime/node-file-system.test.ts`
Expected: PASS（新規 5 ケース + 既存分 すべて）
...
git commit -m "feat(core): FileWriterにrename()を追加しNodeFileSystemで実装"
```
...
gh pr create --draft \
  ...
  --title "feat(core): FileWriterにrename()を追加しNodeFileSystemで実装" \
...
```typescript
   ...
   * Order is preserved; overflow trims from the front (oldest).
   */
...
describe("WisdomPersistence.saveAtomic", () => {
  ...
  it("should propagate rename errors without corrupting the on-disk file", async () => {
    const writer = createMockFileWriter();
    ...
    // target path にはファイルが書かれていない（tmp のみ）
 ...
 * JUSTICE_GLOBAL_WISDOM_PATH environment variable when present, otherwise
 * defaults to `~/.justice/wisdom.json`. Returns null (and logs a warning)
 * when the global store cannot be initialized — callers are expected to
 * fail-open to local-only behavior.
 */
...
export class NoOpPersistence extends WisdomPersistence {
  ...
  override async saveAtomic(): Promise<void> {
    /* no-op */
  }
}
```
...
- `FileWriter.rename(from, to)` interface and `NodeFileSystem.rename()` implementation (path-traversal safe).
...
`README.md` の「コアコンポーネント」相当の箇所（既存の記述に合わせる）に `TieredWisdomStore` / `SecretPatternDetector` を追記。加えて、新規セクションを以下のように追加:
...
`SPEC.md` の §5.15 (`JusticePlugin`) の直後、または適切な `5.x` サブセクションとして以下を追加:
...
- `loadAll()` / `persistAll()` — 両 store を `WisdomPersistence.saveAtomic` で atomic 永続化。

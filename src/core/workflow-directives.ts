...
const DIRECTIVE_TEMPLATES: Readonly<Record<WorkflowDirectiveStage, readonly string[]>> = {
  design_required: [
...
export function formatWorkflowDirective(input: WorkflowDirectiveInput): string {
  const lines = DIRECTIVE_TEMPLATES[input.stage];
  // DIRECTIVE_TEMPLATES covers every WorkflowDirectiveStage exhaustively;
  // the check below is a runtime guard for the impossible case.
  if (lines === undefined) {
    throw new Error(`Unsupported workflow directive stage: ${input.stage}`);
  }
  return lines.join("\n");
}

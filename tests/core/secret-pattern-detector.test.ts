...
describe("SecretPatternDetector", () => {
  ...
  it("should detect linux home paths (with or without trailing slash)", () => {
    expect(detector.scan("error at /home/yohi/.aws/credentials")).toContainEqual({
  ...
  it("should detect macos home paths (with or without trailing slash)", () => {
    expect(detector.scan("error at /Users/alice/Library/")).toContainEqual({
  ...
  it("should detect anthropic API key literal shape", () => {
    expect(detector.scan("sk-ant-abcdefghijklmnopqrstuvwx")).toContainEqual({
      name: "anthropic_key",
    });
    // Should NOT be detected as openai_key
    expect(detector.scan("sk-ant-abcdefghijklmnopqrstuvwx")).not.toContainEqual({
      name: "openai_key",
    });
  });
